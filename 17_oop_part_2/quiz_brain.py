...
class QuizBrain:
  ...
  def populate_list(self, databank):
    """Void. Arg `databank` = [ {'text': '...', 'answer': 'True'/'False'}, ... ]
    ...
    for question in databank:
      self.question_list += [Question(question['text'], question['answer'])]
  ...
  def next_question(self):
    ...
    input_answer = input(f"\nQ.{self.question_number}: {question.text} (True/False)?: ").strip().capitalize()
