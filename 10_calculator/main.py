...
def input_user(input_question, validation_check):
    ...
    if validation_check == "operator":
        while flag:
            data = input(input_question).strip()
            if data in ['+','-','*','/']: flag = False
            else: print(f"Invalid input! '{data}' is not a operator.")
        return data

    while flag:
        data = input(input_question).lower().strip()
        if data in ['y', 'yes', 'n', 'no', 'q', 'quit', 'e', 'exit']: flag = False
    if data in ['y', 'yes']: return 'y'
    if data in ['n', 'no']: return 'n'
    return 'quit'
...
def calculator():
    ...
    while flag:
        ...
        input_operator = input_user("Pick an operator ( + - * / ): ", "operator")
