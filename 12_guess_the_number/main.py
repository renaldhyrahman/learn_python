...
class Game:
    ...
    def input_validation(question, options):
        ...
        if type(question) != str or type(options) not in [list, int]: return None
        result = None
        if type(options) == list:
            while result not in options: result = input(question).strip().lower()
        else:
    ...
    def input_user(self, task):
        ...
        if task not in ["start", "difficulty", "guess", "continue"]: return None
        question = None
        accept = None
        reject = None
        match task:
            ...
            case "continue":
                ...
                reject = ["n", "no"]
        if type(accept) == list:
            if self.input_validation(question, accept + reject) in accept: return True
            else: return False
    ...
    def display(self, task = None):
        """args = task: string
        return = None"""
        if task not in ["welcome", "start", "guess", "win", "lose", "exit"]: return None
        states = self.states
        ...
        match task:
            ...
            case "guess":
                ...
                attempts = f"{states.lives} attempt{'' if states.lives == 1 else 's'}"
                if info:
                    high_number = []
                    low_number = []
                    for number in self.past_guess:
                        if number > states.secret_number: high_number += [number]
                        else: low_number += [number]
                    if info == "high": print(f"\n         Too High.")
                    if info == "low": print(f"\n         Too Low.")
                    hint = ''
                    if len(low_number): hint += f' higher than {max(low_number)}'
                    if len(low_number) and len(high_number): hint += ','
                    if len(high_number): hint += f' lower than {min(high_number)}'
                    print(f"Hint:{hint}")
                print(f"\nYou have {attempts} remaining to guess the number.")
            case "win":
                print("         YOU WIN !")
                print(f"You guessed the correct Number! ({states.secret_number})")
                print("\nPlay another round?")
            case "lose":
                print("         YOU LOSE !")
    ...
    def create_secret_number(self):
        states = self.states
        states.secret_number = random.choice(range(states.range_number[0], states.range_number[1] + 1))
    ...
    def is_player_win(self):
        if self.states.player_win: self.display("win")
        else: self.display("lose")
    ...
    def start(self):
        ...
        self.display("start")
        self.set_difficulty()
        while states.is_playing: self.game_logic()
        self.is_player_win()
        if not self.input_user("continue"): states.continue_playing = False
        else:
            self.reset()
            self.create()
...
def guess_a_number():
    ...
    while game.states.continue_playing: game.start()
    game.display("exit")
