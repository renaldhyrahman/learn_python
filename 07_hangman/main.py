...
# TODO-3: - Import the logo from hangman_art.py and print it at
...
# TODO-6: - Update the code below to tell the user
...
def game_logic(data):
    ...
    # Branch 1: lives > 0 (game continues)
        ...
        player_input = input("Guess a letter: ").strip().upper()
        # Input validation
        if len(player_input) != 1:
            game = game._replace(info="\n****    Invalid input !    ****\n")
        elif player_input in player.history:
            game = game._replace(
                info=f"\n**   [ {player_input} ] Already guessed, "
            ...
            # Branch 2-0: Correct Guess
                ...
                player = player._replace(
                    guess_field=update_guess_field(game.word, player.history)
                )
...
def display(data):
    ...
    if not game.info:
        ...
        text_header = game.info

    # Display updated current_lives
    display_lives = ""
    for _ in range(game.max_lives - player.lives):
        display_lives += "🖤"
    ...
    if not game.over:
        ...
        text_footer = f"Word was [ {game.word} ], wrong guess: {wrong_guess}"
