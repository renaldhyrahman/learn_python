...
class App:
    ...
    def get_choice(self):
        ...
        while not choice:
            choice = (
                input(f"What would you like? ({self.Menu.get_items()}): ")
                .strip()
                .lower()
            )
            if choice in self.MAINTENANCE:
                break
    ...
    def run(self):
        ...
        if choice == "report":
            self.print_report()
            return
        if not self.CoffeeMaker.is_resource_sufficient(choice):
            return
        invalid_coin = True
        while invalid_coin:
            try:
                if not self.MoneyMachine.make_payment(choice.cost):
                    return
                invalid_coin = False
            except ValueError:
                print("Invalid coin")
