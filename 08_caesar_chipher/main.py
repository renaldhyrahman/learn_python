...
def caesar(data_list):
    ...
    result = ''
    for letter in text:
        if letter not in dictionary: result += letter
        else:
            if direction == 'encode': shift_amount = shift
            if direction == 'decode': shift_amount = shift * -1
            result += dictionary[(dictionary.index(letter) + shift_amount) % len(dictionary)]
